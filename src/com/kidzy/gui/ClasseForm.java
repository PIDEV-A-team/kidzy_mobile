 ...
 * @author Asus X550V
 */
public class ClasseForm extends BaseForm {
...
public ClasseForm(){
    ...
    protected boolean isCurrentClasse() {
        return true; //To change body of generated methods, choose Tools | Templates.
    }
    
    public ClasseForm (com.codename1.ui.util.Resources resourceObjectInstance) {
        getToolbar().addCommandToRightBar("", resourceObjectInstance.getImage("toolbar-profile-pic.png"), e -> {});
        installSidemenu(resourceObjectInstance);
        // initGuiBuilderComponents(resourceObjectInstance);
        getToolbar().setTitleComponent(
                        FlowLayout.encloseRight(
                        new Label("Les classes", "Title")
                ));
                        setLayout(BoxLayout.y());
        setLayout(BoxLayout.y());
    for(int i=0;i<ServiceClasse.getInstance().getAllClasses().size();i++){
    Container cy = new Container(BoxLayout.y());
    Container cyy = new Container(BoxLayout.y());
    Container cyyy = new Container(BoxLayout.y());
   ...
   placeholder = EncodedImage.createFromImage(Image.createImage(1100, 500), false);
        Image img = URLImage.createToStorage(placeholder,"http://localhost/classes/"+ServiceClasse.getInstance().getAllClasses().get(i).getImage(), "http://localhost/classes/"+ServiceClasse.getInstance().getAllClasses().get(i).getImage(),URLImage.RESIZE_SCALE);

        Label titre = new Label(ServiceClasse.getInstance().getAllClasses().get(i).getLibelle_cla());
                titre.setUIID("MultiLine2");
        titre.getAllStyles().setFgColor(0x0000ff);
        int id=ServiceClasse.getInstance().getAllClasses().get(i).getId_classe();
        titre.addPointerPressedListener(new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent evt) {
        new OclasseForm(id).show();
        }
    });
        Label cat = new Label(ServiceClasse.getInstance().getAllClasses().get(i).getLibelle_cla());
        cat.getAllStyles().setFgColor(0xffce9b10);
        cxx.add(img).add(titre);
        cxx.setLeadComponent(titre);

        cx.add(cat);
        cy.add(cxx).add(cx);
    
        //cy.setLeadComponent(titre);
        this.add(cy);
    }
